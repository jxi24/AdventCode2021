...
std::vector<LanternFish> Reduce(const std::vector<LanternFish> &fish) {
    std::vector<LanternFish> results;
    for(size_t i = 0; i < 9; ++i) {
        results.emplace_back(i, 0); 
    }

    for(size_t i = 0; i < fish.size(); ++i) {
        results[fish[i].Age()].Weight() += fish[i].Weight();
    }

    return results;
}

size_t Count(const std::vector<LanternFish> &fish) {
    size_t count = 0;
    for(size_t i = 0; i < fish.size(); ++i) {
        count += fish[i].Weight(); 
    }
...
int main() {
    ...
    for(const auto &f : fish) {
        std::cout << f.Age() << " " << f.Weight() << std::endl;
    }

    std::cout << "There are " << Count(fish) << " fish afer 256 days!" << std::endl;
    return 0;
}
